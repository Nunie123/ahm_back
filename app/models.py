...
class User(UserMixin, db.Model):
    ...
    is_email_authenticated = db.Column(db.Boolean, server_default=False)
    ...
    @classmethod
    def add_new_user(cls, email, username, password, is_admin):
        user = cls(email=email, username=username, is_admin=is_admin)
...
class GeoCode(db.Model):
    ...
    geographic_level = db.Column(sa.enum('state', 'county', name='geo_levels', create_type=True)\
        , nullable=False)
    datasets = db.relationship('GeographicDataset', backref='geo_code', lazy='dynamic')
    maps = db.relationship('Map', backref='geo_code', lazy='dynamic')
...
class GeographicDataset(db.Model):
    ...
    is_public = db.Column(db.Boolean, server_default=True)
    created_at = db.Column(db.DateTime, server_default=sa.func.now())
...
class GeographicAttribute(db.Model):
    ...
    geo_code_id = db.Column(db.Text, db.ForeignKey('geo_codes.geo_code'), nullable=False)
    dataset_id = db.Column(db.Integer, db.ForeignKey('geographic_datasets.geographic_dataset_id')\
        , nullable=False)
    attribute_name = db.Column(db.Text, nullable=False)
    attribute_value = db.Column(db.Numeric, nullable=False)
    attribute_relative_weight = db.Column(sa.enum('high', 'medium', 'low'), name='relative_weights'\
        , create_type=True)
...
class Map(db.Model):
    ...
    dataset_1_id = db.Column(db.Integer, db.ForeignKey('geographic_datasets.geographic_dataset_id')\
        , nullable=False)

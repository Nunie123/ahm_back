...
function addChoroplethToMap(map, geoJson, propertyJson){

    var propertyName = propertyJson[0]['attribute_name'];

    addPropertyToGeoJson(geoJson, propertyJson);
    ...
    function style(feature) {
        var fillColor = getColor(feature, propertyName)
        return {
            fillColor: fillColor,
            weight: 2,
            opacity: 0.3,
    ...
    function getColor(feature, propertyName, colors=['#1F9FFC', '#8ccfff', '#e0f2ff']){
        var rank = feature.properties[`${propertyName} relative rank`]
        var color = rank == 'high' ? colors[0] : 
                rank == 'medium' ? colors[1] : colors[2];
        return color;
    }
...
function selectAttribute(e){
    ...
    fetch(url)
        ...
        .then(function(propertyJson) {
            addChoroplethToMap(map, usStateGeoJson, propertyJson)
        });
}
